from __future__ import annotations

from datetime import date, datetime
from typing import List, Optional
from typing_extensions import Literal

from pydantic import BaseModel, EmailStr, Field


class EmployeeCreate(BaseModel):
    employee_code: str = Field(min_length=1, max_length=64)
    name: str = Field(min_length=1, max_length=120)
    fixed_start_time: Optional[str] = None
    hire_date: date
    termination_date: Optional[date] = None
    title: Literal["GM", "AM", "STAFF", "OTHER"] = "STAFF"
    work_group: Literal["FRONT", "BACK"] = "FRONT"
    admin_pin: Optional[str] = None


class EmployeeOut(BaseModel):
    id: str
    employee_code: str
    name: str
    fixed_start_time: Optional[str]
    hire_date: date
    termination_date: Optional[date]
    title: str
    work_group: str
    is_active: bool

    class Config:
        orm_mode = True


class EventCreate(BaseModel):
    employee_code: str
    event_type: Literal["CLOCK_IN", "CLOCK_OUT", "BREAK_START", "BREAK_END"]
    ts_utc: Optional[datetime] = None
    event_uuid: Optional[str] = Field(default=None, min_length=8, max_length=64)
    method: Literal["FACE", "MANUAL", "ADMIN_EDIT"] = "FACE"
    confidence: Optional[float] = None
    device_id: str
    admin_pin: Optional[str] = None
    note: Optional[str] = None


class EventOut(BaseModel):
    id: str
    employee_id: str
    event_type: str
    ts_utc: datetime
    effective_ts_utc: Optional[datetime] = None
    event_uuid: Optional[str]
    method: str
    confidence: Optional[float]

    class Config:
        orm_mode = True


class PayPeriodRequest(BaseModel):
    start_date: date
    end_date: date


class EmailCsvRequest(PayPeriodRequest):
    to_email: EmailStr


class IdentifyRequest(BaseModel):
    embedding: List[float]
    threshold: float = 0.53


class IdentifyResponse(BaseModel):
    matched: bool
    employee_code: Optional[str]
    employee_name: Optional[str]
    confidence: float
    distance: Optional[float]
    status: Optional[str] = None
    allowed_events: Optional[List[str]] = None
    reason: Optional[str] = None
    second_distance: Optional[float] = None


class OfflineFaceCacheEmployee(BaseModel):
    employee_code: str
    employee_name: str
    embeddings: List[List[float]]


class OfflineFaceCacheResponse(BaseModel):
    generated_at: datetime
    offline_threshold: float = 0.53
    total_employees: int
    total_templates: int
    employees: List[OfflineFaceCacheEmployee]


class FaceRegisterRequest(BaseModel):
    employee_code: str
    embeddings: List[List[float]]
    admin_pin: str
    replace_existing: bool = True
    force: bool = False


class FaceRegisterResponse(BaseModel):
    ok: bool
    employee_code: str
    template_count: int


class AdminPinVerifyRequest(BaseModel):
    admin_pin: str


class AdminPinVerifyResponse(BaseModel):
    ok: bool


class AdminPinChangeRequest(BaseModel):
    admin_pin: str
    new_admin_pin: str = Field(min_length=4, max_length=64)


class AdminPinChangeResponse(BaseModel):
    ok: bool


class AdminEmployeeRow(BaseModel):
    id: str
    employee_code: str
    name: str
    fixed_start_time: Optional[str]
    hire_date: date
    termination_date: Optional[date]
    title: str
    work_group: str
    is_active: bool
    face_template_count: int


class AdminEmployeeUpdateRequest(BaseModel):
    admin_pin: str
    name: str = Field(min_length=1, max_length=120)
    fixed_start_time: Optional[str] = None
    hire_date: date
    termination_date: Optional[date] = None
    title: Literal["GM", "AM", "STAFF", "OTHER"] = "STAFF"
    work_group: Literal["FRONT", "BACK"] = "FRONT"
    is_active: bool = True


class AdminEmployeeUpdateResponse(BaseModel):
    ok: bool


class AdminRecentEvent(BaseModel):
    employee_code: str
    employee_name: str
    event_type: str
    method: str
    ts_utc: datetime


class AdminOverviewResponse(BaseModel):
    active_employees: int
    registered_face_employees: int
    total_face_templates: int
    today_events: int
    recent_events: List[AdminRecentEvent]


class EmployeeBulkImportResponse(BaseModel):
    created: int
    updated: int
    skipped: int
    errors: List[str]


class AdminPayrollAutoEmailStatusResponse(BaseModel):
    enabled: bool
    to_email: Optional[EmailStr] = None
    schedule: str
    cycle_weeks: int = Field(default=2, ge=1, le=2)
    anchor_weekday: int = Field(default=0, ge=0, le=6)  # 0=Mon ... 6=Sun
    anchor_start_date: Optional[date] = None
    catchup_once: bool = True
    pending_cycle_weeks: Optional[int] = Field(default=None, ge=1, le=2)
    pending_anchor_weekday: Optional[int] = Field(default=None, ge=0, le=6)
    pending_anchor_start_date: Optional[date] = None
    pending_effective_start: Optional[date] = None
    next_start_date: Optional[date] = None
    next_end_date: Optional[date] = None
    next_send_at_et: Optional[datetime] = None
    last_sent_end: Optional[date] = None
    last_error: Optional[str] = None


class AdminPayrollAutoEmailConfigRequest(BaseModel):
    admin_pin: str
    enabled: bool = False
    to_email: Optional[EmailStr] = None
    cycle_weeks: int = Field(default=2, ge=1, le=2)
    anchor_weekday: int = Field(default=0, ge=0, le=6)  # 0=Mon ... 6=Sun
    anchor_start_date: Optional[date] = None
    catchup_once: bool = True


class AdminPayrollAutoEmailConfigResponse(BaseModel):
    ok: bool


class AdminResetRequest(BaseModel):
    admin_pin: str
    reset_password: str


class AdminResetResponse(BaseModel):
    ok: bool
    deleted_employees: int
    deleted_face_templates: int
    deleted_time_events: int
    deleted_time_segments: int
    deleted_audit_logs: int


class AdminSystemBackupItem(BaseModel):
    backup_id: str
    kind: str
    filename: str
    backup_year: int
    covered_start: str
    covered_end: str
    created_at: str
    size_bytes: int
    target: Optional[str] = None


class AdminSystemBackupListResponse(BaseModel):
    items: List[AdminSystemBackupItem]


class AdminSystemBackupRunRequest(BaseModel):
    admin_pin: str
    backup_password: str


class AdminSystemBackupRunResponse(BaseModel):
    ok: bool
    annual: AdminSystemBackupItem
    recovery: AdminSystemBackupItem


class AdminSoftwareBackupRunRequest(BaseModel):
    admin_pin: str
    backup_password: str
    target: Literal["t", "time", "cloudfront"]


class AdminSoftwareBackupRunResponse(BaseModel):
    ok: bool
    created: AdminSystemBackupItem


class AdminSystemBackupRestoreRequest(BaseModel):
    admin_pin: str
    backup_password: str
    backup_id: str


class AdminSystemBackupRestoreResponse(BaseModel):
    ok: bool
    restored: AdminSystemBackupItem


class AdminSystemBackupDeleteRequest(BaseModel):
    admin_pin: str
    backup_password: str
    backup_id: str


class AdminSystemBackupDeleteResponse(BaseModel):
    ok: bool
    deleted: AdminSystemBackupItem


class AdminSystemBackupUploadResponse(BaseModel):
    ok: bool
    uploaded: AdminSystemBackupItem


class AdminQueryEvent(BaseModel):
    ts_utc: datetime
    ts_et: str
    employee_code: str
    employee_name: str
    title: str
    event_type: str
    method: str


class AdminQueryDay(BaseModel):
    work_date: date
    work_minutes: int
    break_minutes: int
    flags: List[str]


class AdminQuerySummary(BaseModel):
    employee_code: str
    employee_name: str
    title: str
    total_work_minutes: int
    total_break_minutes: int
    days: List[AdminQueryDay]


class AdminEventQueryResponse(BaseModel):
    start_date: date
    end_date: date
    scope: Literal["ALL", "EMPLOYEE_CODE", "EMPLOYEE_NAME"] = "ALL"
    event_filter: Literal["ALL", "CLOCK", "BREAK"] = "ALL"
    q: str = ""
    page: int = 1
    page_size: int = 30
    total_events: int
    total_pages: int
    matched_employees: int
    summaries: List[AdminQuerySummary]
    events: List[AdminQueryEvent]


class AdminEventQueryEmailRequest(BaseModel):
    admin_pin: str
    start_date: str
    end_date: str
    q: str = ""
    scope: Literal["ALL", "EMPLOYEE_CODE", "EMPLOYEE_NAME"] = "ALL"
    event_filter: Literal["ALL", "CLOCK", "BREAK"] = "ALL"
